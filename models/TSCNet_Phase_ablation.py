...
# (1) Hilbert Phase
def _extract_phase_hilbert(self, x: torch.Tensor) -> torch.Tensor:
    analytic = hilbert_transform(x)          # [B, L, C], complex
    phase = torch.angle(analytic)            # wrapped phase in [-pi, pi)
    return phase

# (2) Fourier phase
def _extract_phase_fourier(self, x: torch.Tensor) -> torch.Tensor:
    # x: [B, L, C]
    B, L, C = x.shape
    Xf = torch.fft.rfft(x, dim=1)            # [B, F, C]
    amp = torch.abs(Xf).clone()
    amp[:, 0, :] = 0.0                       # 去掉直流分量

    k_idx = torch.argmax(amp, dim=1)         # [B, C]
    theta = torch.gather(
        torch.angle(Xf), 1, k_idx.unsqueeze(1)
    ).squeeze(1)                             # [B, C]

    t = torch.arange(L, device=x.device, dtype=x.dtype).view(1, L, 1)   # [1, L, 1]
    k_float = k_idx.to(x.dtype).unsqueeze(1)                              # [B, 1, C]
    theta = theta.to(x.dtype).unsqueeze(1)                                # [B, 1, C]

    phase = 2.0 * math.pi * k_float * t / L + theta
    phase = wrap_to_pi(phase)
    return phase

# (3) Sinusoidal phase
def _extract_phase_sinusoidal(self, x: torch.Tensor) -> torch.Tensor:
    B, L, C = x.shape
    t = torch.arange(L, device=x.device, dtype=x.dtype).view(1, L, 1)
    phase = 2.0 * math.pi * t / L            # 固定单周期相位
    phase = phase.expand(B, L, C)
    phase = wrap_to_pi(phase)
    return phase

# (4) Learnable frequency encoding
def _extract_phase_learnable(self, x: torch.Tensor) -> torch.Tensor:
    B, L, C = x.shape
    t = torch.arange(L, device=x.device, dtype=x.dtype).view(1, L, 1)
    omega = F.softplus(self.learnable_omega_raw).view(1, 1, C) + 1e-6
    bias = self.learnable_phase_bias.view(1, 1, C)
    phase = omega * t + bias                 # [1, L, C]
    phase = phase.expand(B, L, C)
    phase = wrap_to_pi(phase)
    return phase

def extract_phase(self, x: torch.Tensor) -> torch.Tensor:
    if self.phase_type == 'hilbert':
        return self._extract_phase_hilbert(x)
    elif self.phase_type == 'fourier':
        return self._extract_phase_fourier(x)
    elif self.phase_type in ['sin', 'sinusoidal']:
        return self._extract_phase_sinusoidal(x)
    elif self.phase_type in ['learnable', 'learnable_freq']:
        return self._extract_phase_learnable(x)
    else:
        raise ValueError(f"Unsupported phase_type: {self.phase_type}")
...
class TSCoupler(nn.Module):
    ...
    def __init__(self,
                 ...
                 C: int = 21,
                 phase_corr_k: int = 15,
                 phase_type: str = 'hilbert'): # Hilbert / Fourier / Sinusoidal / Learnable

        self.phase_type = phase_type.lower()
        # learnable frequency encoding 用到的参数
        self.learnable_omega_raw = nn.Parameter(torch.full((C,), -2.0))
        self.learnable_phase_bias = nn.Parameter(torch.zeros(C))
        ...
        self.extract_phase = extract_phase(phase_type)
    ...
    def forward(self, seasonal_init: torch.Tensor, trend_init: torch.Tensor):
        ...
        # 2) 相位提取（可切换 Hilbert / Fourier / Sinusoidal / Learnable）
        phase = self.extract_phase(seasonal_init)
...
class DCM(nn.Module):
    def __init__(self, configs):
        ...
        self.TS_Coupled = TSCoupler(
            C=self.enc_in,
            phase_type=getattr(configs, 'phase_type', 'hilbert')
        )
