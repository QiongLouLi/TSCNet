...
class FEAM(nn.Module):
    def __init__(self, configs):
        ...
        self.dropout = configs.dropout
        self.seq_len = configs.seq_len
        self.heads = configs.num_head
        self.head_drop = configs.head_drop

        # self.channelAggregator = nn.MultiheadAttention(embed_dim=self.d_model, num_heads=16, batch_first=True,dropout=0.5)
        self.channelAggregator = nn.MultiheadAttention(embed_dim=self.d_model, num_heads=self.heads, batch_first=True,dropout=self.head_drop)
